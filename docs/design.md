...
3. **Dead / unused manifest fields.** `storageWindowHU` is written by every builder but is not in the TS `Manifest` type and is never read (preprocessing metadata only). Top-level `meshes` is always `null`. → decide keep-and-document vs remove. (Manifest `hasSegmentation` *is* read — `App.tsx`/`ControlRail.tsx` gate the segmentation UI on it — so it stays.)
